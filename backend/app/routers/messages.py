...
def _generate(chat_id: str, body: MessageIn, user_id: str):
    ...
    # Keep simple answers compact, but allow substantially richer responses.
    # Provider/model hard limits are still enforced by the provider.
    concise_limit = int(__import__("os").environ.get("LLM_CONCISE_MAX_OUTPUT_TOKENS", "2048"))
    full_limit = int(__import__("os").environ.get("LLM_MAX_OUTPUT_TOKENS", "4096"))
    max_tokens = concise_limit if body.concise else full_limit
